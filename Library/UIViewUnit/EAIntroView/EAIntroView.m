...
- (id)initWithCoder:(NSCoder *)aDecoder {
  ...
  if (self) {
    [self applyDefaultsToSelfDuringInitializationWithFrame:self.frame
                                                     pages:nil];
  }
  return self;
}
...
- (void)applyDefaultsToSelfDuringInitializationWithFrame:(CGRect)frame
                                                   pages:(NSArray *)pagesArray {
...
- (void)applyDefaultsToBackgroundImageView:(UIImageView *)backgroundImageView {
  ...
  backgroundImageView.autoresizesSubviews =
  ...
  [UIView animateWithDuration:0.3
                   animations:^{
                     for (int idx = 0; idx < _pages.count; idx++) {
                       if (idx == panelIndex) {
                         [[self viewForPageIndex:idx]
                             setAlpha:[self alphaForPageIndex:idx]];
                       } else {
...
- (void)checkIndexForScrollView:(EARestrictedScrollView *)scrollView {
  ...
  [self notifyDelegateWithPreviousPage:self.currentPageIndex
                        andCurrentPage:newPageIndex];
...
- (void)finishIntroductionAndRemoveSelf {
  ...
  // Calling removeFromSuperview from scrollViewDidEndDecelerating: method leads
  // to crash on iOS versions < 7.0.
...
- (void)notifyDelegateWithPreviousPage:(NSUInteger)previousPageIndex
                        andCurrentPage:(NSUInteger)currentPageIndex {
  if (currentPageIndex != _currentPageIndex &&
      currentPageIndex < _pages.count) {

    // modify by polen v1.0.3
    EAIntroPage *previousPage = [self pageForIndex:previousPageIndex];
    if (previousPage.onPageDidDisappear)
      previousPage.onPageDidDisappear();

    if (currentPageIndex < _pages.count) {
      EAIntroPage *currentPage = [self pageForIndex:currentPageIndex];
      if (currentPage.onPageDidAppear)
        currentPage.onPageDidAppear();
    } else {
      // currentPageIndex越界
    ...
    if ([(id)self.delegate
            respondsToSelector:@selector(intro:pageAppeared:withIndex:)]) {
      [self.delegate intro:self
              pageAppeared:[self pageForIndex:currentPageIndex]
                 withIndex:currentPageIndex];
    }
  }
}
...
- (NSUInteger)visiblePageIndex {
  return (NSUInteger)((self.scrollView.contentOffset.x +
                       self.scrollView.bounds.size.width / 2) /
                      self.scrollView.frame.size.width);
}
...
- (void)buildScrollView {
  ...
  for (NSUInteger idx = 0; idx < _pages.count; idx++) {
    EAIntroPage *page = [self pageForIndex:idx];
  ...
  self.scrollView.contentSize =
      CGSizeMake(contentXIndex, self.scrollView.frame.size.height);
...
- (UIView *)viewForPage:(EAIntroPage *)page atXIndex:(CGFloat)xIndex {
  UIView *pageView = [[UIView alloc]
      initWithFrame:CGRectMake(xIndex, 0, self.scrollView.frame.size.width,
                               self.scrollView.frame.size.height)];

  pageView.accessibilityLabel = [NSString
      stringWithFormat:@"intro_page_%lu",
                       (unsigned long)[self.pages indexOfObject:page]];
  ...
  if (page.customView) {
    ...
    [constraints addObjectsFromArray:
                     [NSLayoutConstraint
                         constraintsWithVisualFormat:@"H:|-0-[customView]-0-|"
                                             options:0
                                             metrics:nil
                                               views:@{
                                                 @"customView" : page.customView
                                               }]];
    [constraints addObjectsFromArray:
                     [NSLayoutConstraint
                         constraintsWithVisualFormat:@"V:|-0-[customView]-0-|"
                                             options:0
                                             metrics:nil
                                               views:@{
                                                 @"customView" : page.customView
  ...
  [tapToNextButton addTarget:self
  ...
  [constraints
      addObjectsFromArray:
          [NSLayoutConstraint
              constraintsWithVisualFormat:@"H:|-0-[tapToNextButton]-0-|"
                                  options:0
                                  metrics:nil
                                    views:@{
                                      @"tapToNextButton" : tapToNextButton
                                    }]];
  [constraints
      addObjectsFromArray:
          [NSLayoutConstraint
              constraintsWithVisualFormat:@"V:|-0-[tapToNextButton]-0-|"
                                  options:0
                                  metrics:nil
                                    views:@{
                                      @"tapToNextButton" : tapToNextButton
  ...
  if (page.titleIconView) {
        ...
        addConstraints:
            [NSLayoutConstraint
                constraintsWithVisualFormat:
                    @"V:|-topSpace@250-[titleImageView(imageHeight)]"
                                    options:NSLayoutFormatAlignAllTop
                                    metrics:@{
                                      @"imageHeight" : @(
                                          page.titleIconView.frame.size.height),
                                      @"topSpace" : @(page.titleIconPositionY)
                                    } views:@{
    ...
    [pageView
        addConstraints:
            [NSLayoutConstraint
                constraintsWithVisualFormat:@"H:[titleImageView(imageWidth)]"
                                    options:0
                                    metrics:@{
                                      @"imageWidth" :
                                          @(page.titleIconView.frame.size.width)
                                    } views:@{
                                      @"titleImageView" : titleImageView
                                    }]];
  ...
  if (page.title.length) {
    ...
    NSLayoutConstraint *weakConstraint =
        [NSLayoutConstraint constraintWithItem:pageView
    ...
    [pageView addConstraints:
                  [NSLayoutConstraint
                      constraintsWithVisualFormat:@"H:|-10-[titleLabel]-10-|"
                                          options:NSLayoutFormatAlignAllTop
                                          metrics:nil
                                            views:@{
  ...
  if (page.desc.length) {
    ...
    [pageView
        addConstraints:[NSLayoutConstraint
                           constraintsWithVisualFormat:@"H:|-[descLabel]-|"
                                               options:NSLayoutFormatAlignAllTop
                                               metrics:nil
                                                 views:@{
                                                   @"descLabel" : descLabel
  ...
  if (titleImageView && titleLabel && descLabel) {
    [pageView
        addConstraints:
            [NSLayoutConstraint
                constraintsWithVisualFormat:
                    @"V:|->=0-[titleImageView]->=0-[titleLabel]->=0-[descLabel]"
                                    options:0
                                    metrics:nil
                                      views:@{
                                        @"titleImageView" : titleImageView,
...
- (void)appendCloseViewAtXIndex:(CGFloat *)xIndex {
  UIView *closeView = [[UIView alloc]
      initWithFrame:CGRectMake(*xIndex, 0, self.bounds.size.width,
...
- (void)buildTitleView {
  ...
  NSDictionary *metrics = @{
    @"titleViewTopPadding" : @(self.titleViewY),
    @"titleViewHeight" : @(self.titleView.frame.size.height),
    @"titleViewWidth" : @(self.titleView.frame.size.width)
  };

  [self.titleViewConstraints
      addObjectsFromArray:
          [NSLayoutConstraint
              constraintsWithVisualFormat:
                  @"V:|-titleViewTopPadding@250-[titleView(titleViewHeight)]"
  ...
  [self.titleViewConstraints
      addObjectsFromArray:
          [NSLayoutConstraint
              constraintsWithVisualFormat:@"H:[titleView(titleViewWidth)]"
                                  options:NSLayoutFormatAlignAllTop
                                  metrics:metrics
                                    views:views]];
  [self.titleViewConstraints
      addObject:[NSLayoutConstraint constraintWithItem:self.titleView
...
- (void)buildFooterView {
  ...
  NSDictionary *metrics = @{
    @"pageControlBottomPadding" :
        @(self.pageControlY - self.pageControl.frame.size.height),
    @"pageControlHeight" : @(self.pageControl.frame.size.height),
    @"skipButtonBottomPadding" :
        @(self.skipButtonY - self.skipButton.frame.size.height),
    @"skipButtonSideMargin" : @(self.skipButtonSideMargin),
    @"skipButtonWidth" : @(self.skipButton.frame.size.width),
    @"skipButtonHeight" : @(self.skipButton.frame.size.height)
  };
  ...
  [self.footerConstraints
      addObjectsFromArray:
          [NSLayoutConstraint
              constraintsWithVisualFormat:@"V:[pageControl(pageControlHeight)]-"
              @"pageControlBottomPadding@250-|"
  ...
  if (self.skipButton && !self.skipButton.hidden) {
    if (self.skipButtonAlignment == EAViewAlignmentCenter) {
      [self.footerConstraints
          addObjectsFromArray:
              [NSLayoutConstraint
                  constraintsWithVisualFormat:@"H:[skipButton(skipButtonWidth)]"
                                      options:NSLayoutFormatAlignAllTop
                                      metrics:metrics
                                        views:views]];
      [self.footerConstraints
          addObject:[NSLayoutConstraint
                        constraintWithItem:self.skipButton
                                 attribute:NSLayoutAttributeCenterX
                                 relatedBy:NSLayoutRelationEqual
                                    toItem:self
                                 attribute:NSLayoutAttributeCenterX
      ...
      [self.footerConstraints
          addObjectsFromArray:
              [NSLayoutConstraint
                  constraintsWithVisualFormat:
                      @"H:|-skipButtonSideMargin-[skipButton]"
                                      options:NSLayoutFormatAlignAllLeft
                                      metrics:metrics
                                        views:views]];
      ...
      [self.footerConstraints
          addObjectsFromArray:
              [NSLayoutConstraint
                  constraintsWithVisualFormat:
                      @"H:[skipButton]-skipButtonSideMargin-|"
    ...
    if (self.skipButtonY == EA_EMPTY_PROPERTY) {
      [self.footerConstraints
          addObject:[NSLayoutConstraint
                        constraintWithItem:self.pageControl
      ...
      [self.footerConstraints
          addObjectsFromArray:
              [NSLayoutConstraint
                  constraintsWithVisualFormat:@"V:[skipButton(skipButtonHeight)"
                  @"]-skipButtonBottomPadding-|"
                                      options:NSLayoutFormatAlignAllCenterX
                                      metrics:metrics
                                        views:views]];
...
- (void)scrollViewWillBeginDragging:(EARestrictedScrollView *)scrollView {
  if ([self.delegate
          respondsToSelector:@selector(intro:pageStartScrolling:withIndex:)] &&
      self.currentPageIndex < [self.pages count]) {
    [self.delegate intro:self
        pageStartScrolling:[self pageForIndex:_currentPageIndex]
                 withIndex:self.currentPageIndex];
  }
}
...
- (void)scrollViewDidEndDecelerating:(EARestrictedScrollView *)scrollView {
  ...
  if ([self.delegate
          respondsToSelector:@selector(intro:pageEndScrolling:withIndex:)] &&
      self.currentPageIndex < [self.pages count]) {
    [self.delegate intro:self
        pageEndScrolling:[self pageForIndex:_currentPageIndex]
               withIndex:self.currentPageIndex];
  }
}
...
- (void)scrollViewDidScroll:(EARestrictedScrollView *)scrollView {
  ...
  if (page == (_pages.count - 1) && self.swipeToExit) {
    self.alpha = ((self.scrollView.frame.size.width * _pages.count) -
...
- (void)crossDissolveForOffset:(CGFloat)offset {
  ...
  self.pageBgBack.alpha =
      MIN(backLayerAlpha, [self alphaForPageIndex:page + 1]);
  self.pageBgFront.alpha = MIN(frontLayerAlpha, [self alphaForPageIndex:page]);

  if (self.titleView) {
    if ([self showTitleViewForPage:page] &&
        [self showTitleViewForPage:page + 1]) {
...
- (void)deviceOrientationDidChange:(NSNotification *)notification {
  ...
  CGSize newContentSize =
      CGSizeMake(numberOfPages * self.scrollView.frame.size.width,
                 self.scrollView.frame.size.height);
  if (self.scrollView.contentOffset.x > newContentSize.width) {
    CGPoint newOffset = self.scrollView.contentOffset;
    if (self.swipeToExit) {
      newOffset.x =
          newContentSize.width - (self.scrollView.frame.size.width * 2);
    } else {
  ...
  for (EAIntroPage *page in _pages) {
    page.pageView.frame = CGRectMake(i * self.scrollView.bounds.size.width, 0,
  ...
  CGRect pageRect = {.origin.x = offset,
                     .origin.y = 0.0,
  ...
  if (!self.scrollingEnabled) {
    self.scrollView.restrictionArea = CGRectMake(
        self.visiblePageIndex * self.bounds.size.width, 0,
...
- (void)addMotionEffectsOnBg {
  ...
  CGRect parallaxFrame = CGRectMake(
      -self.motionEffectsRelativeValue, -self.motionEffectsRelativeValue,
  ...
  verticalMotionEffect.minimumRelativeValue =
      @(self.motionEffectsRelativeValue);
  verticalMotionEffect.maximumRelativeValue =
      @(-self.motionEffectsRelativeValue);
  ...
  horizontalMotionEffect.minimumRelativeValue =
      @(self.motionEffectsRelativeValue);
  horizontalMotionEffect.maximumRelativeValue =
      @(-self.motionEffectsRelativeValue);
...
- (void)showFullscreenWithAnimateDuration:(CGFloat)duration
  ...
  [self showInView:selectedView
           animateDuration:duration
      withInitialPageIndex:initialPageIndex];
}
...
- (void)showInView:(UIView *)view
         animateDuration:(CGFloat)duration
    withInitialPageIndex:(NSUInteger)initialPageIndex {
  ...
  [UIView animateWithDuration:duration
      ...
      completion:^(BOOL finished) {
        EAIntroPage *currentPage = [self pageForIndex:_currentPageIndex];
        if (currentPage.onPageDidAppear)
          currentPage.onPageDidAppear();
        ...
        if ([(id)self.delegate
                respondsToSelector:@selector(intro:pageAppeared:withIndex:)]) {
          [self.delegate intro:self
                  pageAppeared:[self pageForIndex:_currentPageIndex]
                     withIndex:self.currentPageIndex];
        }
      }];
}
...
- (void)setCurrentPageIndex:(NSUInteger)currentPageIndex
  ...
  if (!animated) {
    // scrollRectVisible:animated: finishes. not called if not animating
    // modify by polen
...
- (void)limitScrollingToPage:(NSUInteger)lastPageIndex {
  ...
  _scrollingEnabled = YES;
  self.scrollView.restrictionArea =
      CGRectMake(0, 0, (lastPageIndex + 1) * self.scrollView.bounds.size.width,
                 self.scrollView.bounds.size.height);
}
