...
#import "MsWGS_84ToGCJ_02.h"

@implementation MsWGS_84ToGCJ_02
...
+ (CLLocation *)transformToMars:(CLLocation *)location {
  ...
  double dLat =
      [[self class] transformLatWithX:location.coordinate.longitude - 105.0
                                    y:location.coordinate.latitude - 35.0];
  double dLon =
      [[self class] transformLonWithX:location.coordinate.longitude - 105.0
                                    y:location.coordinate.latitude - 35.0];
  ...
  CLLocationCoordinate2D coordinate =
      CLLocationCoordinate2DMake((location.coordinate.latitude + dLat),
...
+ (BOOL)outOfChina:(CLLocation *)location {
  if (location.coordinate.longitude < 72.004 ||
      location.coordinate.longitude > 137.8347) {
    return YES;
  }
  if (location.coordinate.latitude < 0.8293 ||
      location.coordinate.latitude > 55.8271) {
    return YES;
  }
  return NO;
}
